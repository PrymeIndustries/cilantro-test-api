...
class _MasterController extends Controller
{
        ...
        return DB::transaction(function () use ($request) {
            ...
            if (count($new_ays) == 0) {
                ...
                try {
                    ...
                    //
                    if ($sc["settings"]["delete_messages"] == 1) {
                        $active_ay = AcademicYear::where('active', true)->first();
                        $messages = Message::where('academic_year_id', '!=', $active_ay->id)->get();
                    ...
                    if ($sc["settings"]["delete_inboxes"] == 1) {
                        $inboxes = Inbox::where('academic_year_id', '!=', $active_ay->id)->get();
